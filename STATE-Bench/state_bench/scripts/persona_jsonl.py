"""Materialize persona YAML files from a JSONL (persona_yaml column).

Mirrors tau2-bench/scripts/persona_jsonl.py so STATE-Bench and the reddit tau2
pipeline key personas identically: one YAML per row, named by the row's user_id
(the reddit username). Identical filenames => identical eval-dir layout across benches.
"""
...
def persona_stem_from_row(row: dict, index: int) -> str:
    """Derive a stable filename stem for a JSONL persona row (user_id first)."""
